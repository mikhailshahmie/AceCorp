...
main.onAuthStateChanged(main.auth, (user) => {
  if (user) {
    ...
    const q = main.query(
      main.bookingDB,
      main.where("passengerId", "==", user.uid),
      main.where("status", "in", ["waiting", "ongoing"])
    );
    ...
    const q2 = main.query(
      main.bookingDB,
      main.where("driverId", "==", user.uid),
      main.where("status", "==", "ongoing")
    );
    ...
    if (!snapshot.empty) {
      ...
      $("#requestBtn").click((e) => {
        if (confirm("Accept this ride?")) {
          ...
          main
            .updateDoc(main.doc(main.db, "bookings", bookingId), {
              driverId: driverId,
              status: "ongoing",
            })
            .then(() => {
